...
class OurDDataset(Dataset):
    ...
    def __init__(
        ...
        for class_name in class_names:
            root=os.path.join(basepath,class_name)
        ...
        #class_names=class_names[:1]
            ...
            instance_num=int(instance_num*0.04)
            #instance_name.sort(key=lambda l: int(re.findall('\d+', l)[0]))  

            instance_name=np.array(instance_name)[:instance_num]#[np.random.choice(len(instance_name),size=[1000],replace=False)]
