...
def main():
    ...
    all_labels = []
    if args.class_cond:
        allclasses = th.arange(0, 100, device=th.device("cpu"))
    ckpt = th.load(args.gt_path)
    tri_plane = ckpt['network_fn_state_dict']['tri_planes'].to(device=th.device("cpu"))

    noise1=th.randn(1,18,256,256).to(device=th.device("cpu"))

    noise2=th.randn(1,18,256,256).to(device=th.device("cpu"))
    ...
    for i in range(args.num_samples):
        ...
        if args.class_cond:
            #ipdb.set_trace()
            classes = allclasses[np.random.randint(1,size=1)].repeat(args.batch_size)
        ...
        sample = sample_fn(
            ...
            noise=noise1*ratio+noise2*(1-ratio),
...
def create_argparser():
    defaults = dict(
        ...
        batch_size=1,
        use_ddim=True,
        model_path="/mnt/petrelfs/caoziang/3D_generation/Checkpoint_all/diffusion_shapenet_new_v1_1/ema_0.9999_400000.pt",
        gt_path='/mnt/petrelfs/caoziang/3D_generation/Checkpoint_all/shapenet_triplane_new/003000.tar',
        save_path='/mnt/petrelfs/caoziang/3D_generation/Checkpoint_all/diffusion_shapenet_new_v1_1',
    )
